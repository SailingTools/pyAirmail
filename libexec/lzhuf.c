...
int Decode(int usock, char* iFile, char* oFile, struct lzhufstruct *lzhuf, int b2f)
{
   ...
   //printf("input file size %ld, fbb_filesize %ld\n", filesize, fbb_filesize);
	...
	if (b2f)
	{
		 ...
		 */
   		if (fread (&crc, sizeof(crc), 1, lzhuf->iFile) < 1)
			...
			//printf("could not read crc value\n");
		/*
		else
			log (-1, "incoming crc %d", crc);
			//printf("incoming crc %d\n", crc");
		*/
   ...
   || (fbb_filesize == 0)) {
      //printf("Error with fbb_filesize\n");
      i=errno;
      if (!i) i--;  /* some non-zero value */
      fclose(lzhuf->iFile);
      fclose(lzhuf->oFile);
	...
	/* log (-1, "fbb_filesize %d, filesize %ld", fbb_filesize, filesize); */
        //printf("fbb_filesize %d, filesize %ld\n", fbb_filesize, filesize);
...
{
    char  *s;
    struct lzhufstruct *lzs;
    int b2f = 0;

    lzs = AllocStruct();

    if (argc != 5) {
        printf("'lzhuf e file1 file2 b2f' encodes file1 into file2 - b2f is '1' or '0'.\n"
               "'lzhuf d file2 file1 b2f' decodes file2 into file1 - b2f is '1' or '0'.\n");
        return EXIT_FAILURE;
    }
    ...
    if (s = argv[4], strcmp(s, "0")) {
	b2f = 1;
    }

    if (toupper(*argv[1]) == 'E')
        // int usock, char *iFile, char *oFile, struct lzhufstruct *lzhuf, int  b2f
        Encode( 0, argv[2], argv[3], lzs, b2f );
    else
        Decode( 0, argv[2], argv[3], lzs, b2f );
